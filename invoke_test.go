...
func TestInvokeReadGeoFiles(t *testing.T) {
	configJSON := `{
		"routing": {
			"rules": [{
				"type": "field",
				"domain": ["geosite:cn", "ext:custom-site.dat:private"],
				"ip": ["geoip:cn", "ext:custom-ip.dat:private"]
			}]
		}
	}`
	response := invokeForTest(
		t,
		LibXrayMethodReadGeoFiles,
		ReadGeoFilesRequest{ConfigJSON: configJSON},
	)
	if !response.Success {
		t.Fatalf("ReadGeoFiles failed: %s", response.Err)
	}
	files := decodeDataObject[ReadGeoFilesResponse](t, response)
	if got := strings.Join(files.Domain, ","); got != "custom-site.dat,geosite.dat" {
		t.Fatalf("domain files = %q", got)
	}
	if got := strings.Join(files.IP, ","); got != "custom-ip.dat,geoip.dat" {
		t.Fatalf("ip files = %q", got)
	}
}

func TestInvokeQueryStatsHTTP(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		_, _ = w.Write([]byte(`{"uplink":12,"downlink":34}`))
	}))
	defer server.Close()

	response := invokeForTest(
		t,
		LibXrayMethodQueryStats,
		QueryStatsRequest{Server: server.URL},
	)
	if !response.Success {
		t.Fatalf("QueryStats failed: %s", response.Err)
	}
	stats := decodeDataObject[QueryStatsResponse](t, response)
	if stats.Stats != `{"uplink":12,"downlink":34}` {
		t.Fatalf("stats = %q", stats.Stats)
	}
}

func TestInvokeSetMemoryLimit(t *testing.T) {
	previous := debug.SetMemoryLimit(-1)
	defer debug.SetMemoryLimit(previous)

	response := invokeForTest(
		t,
		LibXrayMethodSetMemoryLimit,
		SetMemoryLimitRequest{MemoryMB: 4096},
	)
	if !response.Success {
		t.Fatalf("SetMemoryLimit failed: %s", response.Err)
	}
	requireNoDataObject(t, response)
}

func TestInvokeRuntimeMutationMethodsDispatch(t *testing.T) {
	tests := []struct {
		name    string
		method  LibXrayMethod
		payload any
	}{
		{"query stats by tag", LibXrayMethodQueryStatsByTag, QueryStatsByTagRequest{Tag: "proxy"}},
		{"replace inbound", LibXrayMethodReplaceInbound, InboundJSONRequest{InboundJSON: `{}`}},
		{"replace outbound", LibXrayMethodReplaceOutbound, OutboundJSONRequest{OutboundJSON: `{}`}},
		{"add inbound", LibXrayMethodAddInbound, InboundJSONRequest{InboundJSON: `{}`}},
		{"remove inbound", LibXrayMethodRemoveInbound, TagRequest{Tag: "in"}},
		{"add outbound", LibXrayMethodAddOutbound, OutboundJSONRequest{OutboundJSON: `{}`}},
		{"remove outbound", LibXrayMethodRemoveOutbound, TagRequest{Tag: "out"}},
		{"add route rule", LibXrayMethodAddRouteRule, AddRouteRuleRequest{Rule: `{}`, ShouldAppend: true}},
		{"remove route rule", LibXrayMethodRemoveRouteRule, RemoveRouteRuleRequest{RuleTag: "rule"}},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			response := invokeForTest(t, test.method, test.payload)
			if response.Success {
				t.Fatal("method should fail while Xray is stopped")
			}
			if response.Err == "" {
				t.Fatal("method should return an error")
			}
